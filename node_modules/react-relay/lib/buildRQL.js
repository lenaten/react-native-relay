      ...
      if (isDeprecatedCallWithArgCountGreaterThan(queryBuilder, 0)) {
        ...
        if (GraphQL.isQuery(node) && node.fragments.length === 0) {
          if (!node.fields.every(function (field) {
            ...
            !false ? process.env.NODE_ENV !== 'production' ? invariant(false, 'Relay.QL: Expected query `%s` to be empty. For example, use ' + '`node(id: $id)`, not `node(id: $id) { ... }`.', node.fieldName) : invariant(false) : undefined;
          }
